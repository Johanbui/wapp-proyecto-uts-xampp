<template>
  <div>
    <div class="app-container">
      <el-form
        ref="documentosEditForm"
        :model="documentosEditForm"
        ...
        <el-row :gutter="30">
          <el-col :span="24">
            <el-form-item prop="codigo" label="Codigo">
              <el-input
                ref="codigo"
                v-model="documentosEditForm.codigo"
                placeholder="Codigo"
                name="codigo"
                type="text"
                tabindex="1"
        ...
        <el-row :gutter="30">
          <el-col :span="24">
            <el-form-item prop="fecha" label="Fecha">
              <el-date-picker
                v-model="documentosEditForm.fecha"
                type="date"
                placeholder="Fecha"
              />
            </el-form-item>
...
<script>
import { getOne, update } from '@/api/documentos'
  ...
  methods: {
    fetchData() {
      ...
      getOne(this.id).then((response) => {
        ...
        this.documentosEditForm = { ...response.data }
        this.fileList.push(this.documentosEditForm.file)
      })
    },
    onSubmit() {
      const params = { ...this.documentosEditForm }
      ...
      update(params).then((response) => {
        ...
        this.$router.push({ path: '/documentos' })
      })
    },
    onCancel() {
      this.$router.push({ path: '/documentos' })
    ...
    handleSuccess(res, file) {
      ...
      this.documentosEditForm.file_id = res.file.id
    }
  }
}
</script>
